...
time_value time_value::current()
{
    ...
    int64_t microseconds, ticks = get_ticks() - offset;
    asm volatile("imul %%rbx;" /* rdx:rax = rax * rbx */
                 "idiv %%rcx;" /* rax = rdx:rax / rcx */
                 :"=r"(microseconds)
                 :"a"(ticks), "b"(microseconds_per_second), "c"(ticks_per_second)
    );
#elif defined(_WIN32)
