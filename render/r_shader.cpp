...
result shader::reload()
{
    ...
    if (succeeded(res) && (new_vertex_shader || new_fragment_shader)) {
    ...
    gl::shader& shader) const
{
    ...
    if (!shader.compile_status(info_log)) {
        if (info_log.length()) {
            log::error("shader '^fff%s^xxx' failed to compile with errors:\n%s", filename.c_str(), info_log.c_str());
        } else {
            log::error("shader '^fff%s^xxx' failed to compile\n", filename.c_str());
        }
        return result::failure;
    } else {
        if (info_log.length()) {
            log::warning("shader '^fff%s^xxx' compiled with warnings:\n%s", filename.c_str(), info_log.c_str());
        }
