...
//------------------------------------------------------------------------------
shield::shield(physics::shape const* base, game::ship* owner, uint16_t compartment, int maximum_power)
    : subsystem(owner, compartment, {subsystem_type::shields, maximum_power})
    , _base(base)
    , _strength(static_cast<float>(maximum_power))
